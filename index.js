const sdk = require("node-appwrite");
const cheerio = require("cheerio");

module.exports = async function (req, res) {
  try {
    const payload = req.payload ? JSON.parse(req.payload) : {};
    const url = payload.url;

    if (!url) {
      res.json({ error: "URL gerekli" }, 400);
      return;
    }

    // Appwrite client — environment değişkenlerini otomatik alır
    const client = new sdk.Client();
    client
      .setEndpoint(process.env.APPWRITE_FUNCTION_ENDPOINT)
      .setProject(process.env.APPWRITE_FUNCTION_PROJECT_ID)
      .setKey(process.env.APPWRITE_FUNCTION_API_KEY);

    const databases = new sdk.Databases(client);

    const databaseId = "scraper";      // kendi database ID'in
    const collectionId = "page_text"; // kendi collection ID'in

    // HTML indir
    const response = await fetch(url);
    const html = await response.text();

    // Cheerio ile parse et
    const $ = cheerio.load(html);

    // İlgili selector
    const selector = ".css-18unqsv a:first-child .css-63gy1o span";
    const selectedElement = $(selector);

    const textValue = selectedElement.text().trim();

    if (!textValue) {
      res.json({ error: "Verilen selector ile text bulunamadı." }, 404);
      return;
    }

    // Veritabanına kaydet
    const result = await databases.createDocument(
      databaseId,
      collectionId,
      "unique()",
      {
        url: url,
        selector: selector,
        text: textValue,
        createdAt: new Date().toISOString(),
      }
    );

    res.json({
      message: "Veri başarıyla kaydedildi.",
      data: {
        url: url,
        selector: selector,
        text: textValue,
      },
    });
  } catch (error) {
    res.json({ error: error.message }, 500);
  }
};
